...
def analyze(ts_start, ts_jun, contacts): # Function now accepts contacts
    d = {}
    ...
    if d['group_leaderboard']:
        ...
        merged_senders = {}
        # Use the 'contacts' dictionary passed to this analyze function
        
        for handle_id, msg_count in raw_senders:
            # Resolve the raw ID to the contact's display name using the actual contacts
            contact_name = get_name(handle_id, contacts) 
            
            if contact_name not in merged_senders:
                # Store new sender, preserving the original ID just in case, but using display_name for the slide
    ...
    if gs['count'] > 0:
        ...
        if d['group_leaderboard']:
            ...
            if d['top_group_senders']:
                ...
                # Use the 'display_name' field generated by the forced merge in analyze()
                sender_html = ''.join([
...
def main():
    ...
    # FIX: Call analyze with contacts
